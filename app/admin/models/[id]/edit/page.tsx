...
export default function EditModelPage({ params }: { params: { id: string } }) {
  ...
  const fetchModel = async () => {
    try {
      ...
      if (data.images) {
        setExistingImages(data.images.map((img: any) => ({ id: img.id, url: img.url })))
      }
    } catch (error) {
  ...
  const handleDeleteExistingImage = async (imageId: string) => {
    ...
    try {
      const response = await fetch(`/api/model-images/${imageId}`, {
        method: 'DELETE'
      })
      ...
      <div className="admin-layout">
        ...
        <main className="admin-content">
          ...
          <form onSubmit={handleSubmit} className="admin-form">
            ...
            {images.length > 0 && (
              <div style={{ marginTop: '20px' }}>
                <h3>Новые фотографии (будут добавлены при сохранении):</h3>
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  {images.map((img, idx) => (
